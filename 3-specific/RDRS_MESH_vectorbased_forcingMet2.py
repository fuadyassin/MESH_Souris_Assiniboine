...
def remap_rdrs_climate_data(input_directory, output_directory, input_basin, input_ddb, start_year, end_year):
    ...
    print("Basin Info:")
    print(basin.head(), flush=True)  # Print the first few rows of the basin GeoDataFrame
    print("Longitude:", lon[:5], flush=True)  # Print the first few longitude values
    print("Latitude:", lat[:5], flush=True)  # Print the first few latitude values
    print("Subbasin IDs:", segid[:5], flush=True)  # Print the first few subbasin IDs
...
def process_file(file_path, segid, lon, lat, output_directory):
    ...
    # Close the dataset
    forc.close()
    print(f"Finished processing file: {file_path}" , flush=True)
...
if __name__ == "__main__":
    ...
    if args.command == "all_years":
        ...
        print("Finished all_years", flush=True)
        sys.exit(0)  # Exit after completing the "all_years" task
        ...
        print(f"Finished processing files for year {args.year}", flush=True)
        sys.exit(0)  # Exit after completing the "single_year" task    
    print("No valid command provided.", flush=True)
    sys.exit(1)  # Exit with an error code if no command was matched
